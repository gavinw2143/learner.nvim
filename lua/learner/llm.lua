...
function M.setup(opts)
	...
	if not M.config.api_key or M.config.api_key == "" then
		vim.schedule(function()
			vim.notify(
				"Missing LLM API key. Set $OPENAI_API_KEY or call require('learner').setup({ llm = { api_key = 'KEY' } })",
				vim.log.levels.WARN
			)
		end)
	end
	...
	handle = uv.spawn("curl", {
		...
		if code == 0 then
			...
			if status ~= 200 then
				...
				if ok and data and data.output then
					result = data.output.content.text
				else
					vim.schedule(function()
						vim.notify(body, vim.log.levels.ERROR)
					end)
